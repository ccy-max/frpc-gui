...
async function handleExportBase64() {
  ...
  if (!appStore.frpConfig) { message.warning('没有可导出的配置'); return; }
  base64Text.value = safeEncodeBase64(JSON.stringify(appStore.frpConfig, null, 2));
  base64DialogVisible.value = true;
}

async function handleCopyBase64() {
  try { await writeText(base64Text.value); message.success('已复制'); }
...
async function handleParseBase64() {
  if (!base64Text.value.trim()) { message.warning('请输入配置'); return; }
  try {
    const json = safeDecodeBase64(base64Text.value.trim());
    const config = JSON.parse(json);
    if (!config.server_addr || !config.proxies) { message.error('无效配置'); return; }
...
function handleReset() {
  Modal.confirm({
    ...
    onOk: async () => {
      ...
      if (result.success) message.success('已清空');
...
async function pickFile(field: string, ext: string[]) {
  ...
  if (selected) {
    ...
    if (field === 'certFile') config.tls.cert_file = selected as string;
    if (field === 'keyFile') config.tls.key_file = selected as string;
    if (field === 'caFile') config.tls.trusted_ca_file = selected as string;
  }
}
</script>
        ...
        <a-tab-pane key="paths" tab="路径">
          <a-form layout="vertical" :wrapper-col="{ span: 20 }">
            <a-form-item label="FRP 可执行文件">
              <a-input v-model:value="appStore.frpcPath" placeholder="点击右侧按钮选择" readonly>
                <template #suffix>
          ...
          <a-form layout="vertical" :wrapper-col="{ span: 12 }">
            ...
            <template v-if="tlsEnable">
              <a-form-item label="证书文件">
                <a-input v-model:value="tlsCertFile" placeholder="选择证书文件" readonly>
                  <template #suffix>
              ...
              <a-form-item label="密钥文件">
                <a-input v-model:value="tlsKeyFile" placeholder="选择密钥文件" readonly>
                  <template #suffix>
              ...
              <a-form-item label="CA 文件">
                <a-input v-model:value="tlsCaFile" placeholder="选择 CA 文件" readonly>
                  <template #suffix>
